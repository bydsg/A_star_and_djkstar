...
std::vector<vector<Point>>B_spline_optimization(std::vector<vector<Point>> &all_point_vec){
     std::vector<vector<Point>> path_points_vec;
	 path_points_vec=all_point_vec;
	 all_point_vec.clear();
  for(int i=0;i<path_points_vec.size();i++){
	std::vector<Point> path_points;
		Point p;
		int num=path_points_vec[i].size();

		CPosition *testpt=new CPosition[num];
		for(int j=0;j<num;j++) testpt[j]=CPosition(path_points_vec[i][j].x,path_points_vec[i][j].y);
		int *Intnum=new int[num-1]; 
		for(int c=0;c<num-1;c++){
			Intnum[c]=10;                 //  每一个样条曲线内插入10个点
		}
		int num2=num;
		CBSpline bspline;
		...
		all_point_vec.push_back(path_points);
  }
  
    return all_point_vec;
}
