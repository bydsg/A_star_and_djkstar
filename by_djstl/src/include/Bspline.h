...
extern void B_spline_optimization_road(vector<Road> &roads,int num_road,int num_point);
...
void B_spline_optimization_road(vector<Road> &roads,int num_road,int num_point){
	std::vector<Point> all_point;
	for(int i=0;i<roads[num_road].road_points.size();i++){
		Point p;
		p.x=roads[num_road].road_points[i].x;
		p.y=roads[num_road].road_points[i].y;
	...
	CPosition *testpt = new CPosition[num];
	for (int i = 0; i < num; i++)
		testpt[i] = CPosition(all_point[i].y, all_point[i].x);

	int *Intnum = new int[num - 1];
	for (int i = 0; i < num - 1; i++)
	{
		Intnum[i] = num_point; //  每一个样条曲线内插入10个点
	}
	CBSpline bspline;
	bspline.ThreeOrderBSplineInterpolatePt(testpt, num, Intnum); //  三次B样条曲线
	delete Intnum;
	...
	roads[num_road].road_points.clear();
	for(int i=0;i<path_points.size();i++){
		Point p;
		p.x=path_points[i].y;
		p.y=path_points[i].x;
		roads[num_road].road_points.push_back(p);
	}
}